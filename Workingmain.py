...
- Session via Bot (no console input)
- Mode control: OTP / Private / All
...
SESSION_FILE_GITHUB = os.getenv("SESSION_FILE_GITHUB", "session.txt")

# ---------- MODE MANAGEMENT ----------
MODE_FILE_LOCAL = "mode.txt"

def fetch_file_from_github(path: str):
...
def push_file_to_github(path: str, content: str, msg: str):
    url = f"https://api.github.com/repos/{REPO}/contents/{path}"
    headers = {"Authorization": f"token {GITHUB_TOKEN}"}
    b64_content = base64.b64encode(content.encode()).decode()

    payload = {"message": msg, "content": b64_content, "branch": BRANCH}
    r = requests.get(url, headers=headers, params={"ref": BRANCH})
    if r.status_code == 200:
    ...
    if resp.status_code in (200, 201):
        print(f"✅ GitHub file {path} updated.")
    else:
        print(f"⚠️ GitHub update failed ({resp.status_code}): {resp.text}")

def load_mode():
    if os.path.exists(MODE_FILE_LOCAL):
        return open(MODE_FILE_LOCAL).read().strip()
    mode = fetch_file_from_github(MODE_FILE_GITHUB)
    return mode if mode else "otp"

def save_mode(mode: str):
    with open(MODE_FILE_LOCAL, "w") as f:
        f.write(mode)
    push_file_to_github(MODE_FILE_GITHUB, mode, f"🔄 Mode switched → {mode}")

current_mode = load_mode()

# ---------- CLIENTS ----------
bot_client = TelegramClient("bot_session", API_ID, API_HASH)
SESSION_STRING = os.getenv("SESSION_STRING") or fetch_file_from_github(SESSION_FILE_GITHUB)

# ---------- LOGGER ----------
def update_github_file(new_text: str):
    old_content = fetch_file_from_github(FILE_PATH) or ""
    updated_content = old_content + "\n" + new_text
    push_file_to_github(FILE_PATH, updated_content, "📜 Log update")
...
async def create_session_via_bot():
    ...
    await bot_client.send_message(ADMIN_CHAT_ID, "✅ Session created and saved.")
    push_file_to_github(SESSION_FILE_GITHUB, session_str, "🔑 New session created")
    return session_str
...
async def register_handlers(user_client):
    ...
    async def handle_new_message(event):
        ...
        try:
            ...
            info = f"👤 {sender_name} (ID: {sender_id}, Chat: {chat_id})"
...
# ---------- CLIENTS ----------
bot_client = TelegramClient("bot_session", API_ID, API_HASH)
SESSION_STRING = os.getenv("SESSION_STRING")  # optional: preload from GitHub secrets
