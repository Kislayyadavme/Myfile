"""
message_fetcher.py (Auto Session via Bot + GitHub Sync)
"""
...
SESSION_STRING = os.getenv("SESSION_STRING", None)   # may be empty
...
async def get_input_from_bot(prompt: str):
    """Ask ADMIN_CHAT_ID for input via bot and wait for reply."""
    await bot_client.send_message(ADMIN_CHAT_ID, f"❓ {prompt}")
    event = await bot_client.wait_event(events.NewMessage(from_user=ADMIN_CHAT_ID))
    return event.raw_text.strip()


async def create_session_via_bot():
    """Create new session from GitHub runner IP via bot interaction."""
    ...
    await bot_client.send_message(ADMIN_CHAT_ID, f"✅ Session created:\n\n`{session_str}`\n\n⚠️ Save this in GitHub Secrets as `SESSION_STRING`.")
    return session_str


# Ensure SESSION_STRING exists
